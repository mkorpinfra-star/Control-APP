...
try {
    ...
    if (!empty($input['all'])) {
        ...
        $stmt = $pdo->prepare("DELETE FROM apontamentos WHERE semana_inicio = ?");
        $stmt->execute([$input['semana']]);
