...
def gerar_icone_512():
    ...
    # Criar imagem QUADRADA SEM BORDAS ARREDONDADAS
    ...
    # Fontes
    try:
        font_j2s = ImageFont.truetype("arialbd.ttf", 160)
        font_hores = ImageFont.truetype("arialbd.ttf", 65)
    except:
        try:
            font_j2s = ImageFont.truetype("arial.ttf", 160)
            font_hores = ImageFont.truetype("arial.ttf", 65)
        except:
            font_j2s = ImageFont.load_default()
            font_hores = ImageFont.load_default()

    # Desenhar J2S GRUDADO (letra por letra com espaçamento negativo)
    y_j2s = 140
    letra_j = "J"
    letra_2 = "2"
    letra_s = "S"
    ...
    # J
    bbox_j = draw.textbbox((0, 0), letra_j, font=font_j2s)
    w_j = bbox_j[2] - bbox_j[0]
    x_j = 70
    draw.text((x_j, y_j2s), letra_j, fill=COR_BRANCO, font=font_j2s)

    # 2 (grudado no J)
    bbox_2 = draw.textbbox((0, 0), letra_2, font=font_j2s)
    w_2 = bbox_2[2] - bbox_2[0]
    x_2 = x_j + w_j - 25
    draw.text((x_2, y_j2s), letra_2, fill=COR_BRANCO, font=font_j2s)

    # S (grudado no 2)
    x_s = x_2 + w_2 - 25
    draw.text((x_s, y_j2s), letra_s, fill=COR_BRANCO, font=font_j2s)
    ...
    # Desenhar ÍCONE DE RELÓGIO (canto superior direito)
    clock_x = 400
    clock_y = 100
    clock_radius = 50
    ...
    draw.line([(clock_x, clock_y), (clock_x, clock_y - 35)],
              fill=COR_BRANCO, width=5)

    # Ponteiro dos minutos (3h)
    draw.line([(clock_x, clock_y), (clock_x + 40, clock_y)],
              fill=COR_BRANCO, width=5)

    # Ponto central
    draw.ellipse(
        [(clock_x - 6, clock_y - 6), (clock_x + 6, clock_y + 6)],
        fill=COR_BRANCO
    )
