  ...
  return (
    <div className="bg-[#121419] border border-[#23262E] rounded-2xl p-3 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {total > 0 && <span className="text-sm font-bold text-[#F08020] tabular-nums">{brl(total)}</span>}
      </div>
        ...
        <div className="space-y-1.5">
          {servicos.map(s => (
            <div key={s.id} className="flex items-center justify-between text-sm">
              ...
              <span className="text-[#A8ADB8] tabular-nums mr-2">{brl(s.valor_total)}</span>
      ...
      {tipos.length === 0 ? (
        ...
        <div className="flex gap-2">
          <select value={tipoId} onChange={e => setTipoId(e.target.value)} className={`${ui.input} flex-1 h-10`}>
            ...
            {tipos.map(t => <option key={t.id} value={t.id}>{t.nome} ({brl(t.valor_padrao)})</option>)}
          </select>
